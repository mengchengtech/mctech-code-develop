...
Explore 的职责是发现问题和事实，不是做方案决策。最佳做法是把 Open Questions 分层处理：

| Open Question 类型            | Explore 阶段怎么处理 | 后续阶段          |
| ----------------------------- | -------------------- | ----------------- |
| 当前代码到底怎么实现          | **必须解决**         | —                 |
| 某个调用关系/依赖是否存在     | **尽量解决**         | —                 |
| 当前数据库/API/缓存行为是什么 | **必须确认**         | —                 |
| “应该采用方案 A 还是 B”       | **不要解决**         | Analyze / Decide  |
| “这个权限应该继承还是覆盖”    | **不要自行决定**     | Analyze / Decide  |
| “新需求到底要求什么”          | **不能猜**           | 用户确认 / Decide |
| 缺少业务规则                  | **记录下来**         | Analyze / Spec    |
| 外部系统行为无法确认          | **记录并标记**       | 后续确认          |
...
如果这一步分析后有待决策的内容。不要在当前环节解决，而应该放到 mctech-sdd-decide 去解决
